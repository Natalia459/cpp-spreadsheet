...
inline std::ostream& operator<<(std::ostream& os, FormulaError err) {
	return os << err.ToString();
}

inline std::ostream& operator<<(std::ostream& os, FormulaError::Category err_cat) {
	return os << FormulaError{ err_cat };
}
